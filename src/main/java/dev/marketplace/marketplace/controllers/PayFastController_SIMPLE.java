 ...
 *
 * Signature Rules:
 * - Initial request: include merchant_key, raw values (no encoding), alphabetical order, append passphrase, MD5
 * - ITN callback: exclude merchant_key, raw values (no encoding), alphabetical order, append passphrase, MD5
 */
...
public class PayFastController_SIMPLE {
    ...
    public ResponseEntity<String> getPayFastSubscriptionUrl(
        ...
        // Build redirect URL (with percent-encoded params)
        StringBuilder url = new StringBuilder(payFastProperties.getUrl()).append("?");
        boolean first = true;
        for (Map.Entry<String, String> e : params.entrySet()) {
            if (!first) url.append('&');
            first = false;
            url.append(e.getKey()).append('=').append(encode(e.getValue()));
        }
        url.append("&signature=").append(signature);

        log.info("[PayFast] URL generated");
        return ResponseEntity.ok(url.toString());
    }

    /**
     * Compute PayFast signature.
     * Base string: sorted params (raw values, no encoding) + passphrase, then MD5.
     */
    private String computeSignature(Map<String, String> params, boolean includeMerchantKey) {
        ...
        // Build base string: key=value&key=value&...
        StringBuilder sb = new StringBuilder();
        for (String key : keys) {
            String val = params.get(key);
            if (val == null || val.isEmpty()) continue;
            if (key.equals("merchant_key") && !includeMerchantKey) continue;
        ...
        log.info("[PayFast] Signature base: {}", baseString);

        String md5 = org.apache.commons.codec.digest.DigestUtils.md5Hex(baseString);
        log.info("[PayFast] Signature MD5: {}", md5);
        return md5;
    }
    ...
    public ResponseEntity<String> handlePayFastITN(@RequestParam Map<String, String> payload) {
        ...
        // Compute expected signature (exclude merchant_key for ITN)
        ...
        if ("COMPLETE".equalsIgnoreCase(status) && email != null) {
            Optional<User> userOpt = userService.getUserByEmail(email);
            if (userOpt.isPresent()) {
                try {
                    subscriptionService.createOrActivatePayFastSubscription(userOpt.get().getId(),
                        Subscription.PlanType.valueOf(plan.toUpperCase()));
                    log.info("[PayFast ITN] Subscription activated for {}", email);
                } catch (Exception e) {
                    log.error("[PayFast ITN] Subscription activation failed: {}", e.getMessage());
                }
            }
        }
